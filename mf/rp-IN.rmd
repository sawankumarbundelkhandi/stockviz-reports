...
params:
  LARGECAP_SCHEME_CODES: c()
  MIDCAP_SCHEME_CODES: c()	
  ST_DT: Sys.Date() - 5*365
  ASOF_DT: Sys.Date()
---
...
analyzeFunds <- function(schemeCodes, indices){
	...
	for(benchIndex in 1:length(indices)){
		...
		for(mfIndex in 1:ncol(retXts)){
			...
			lmCoeffs <- c(round(as.numeric(linearFit$coeff), 5), sprintf('<a href="/reports/mf/analysis/rp-%s.html" target="_blank">>>></a>', schemeCodes[mfIndex]))
...
largemfDT <- analyzeFunds(params$LARGECAP_SCHEME_CODES, benchmark_indices[['LARGECAP']])
midmfDT <- analyzeFunds(params$MIDCAP_SCHEME_CODES, benchmark_indices[['MIDCAP']])
